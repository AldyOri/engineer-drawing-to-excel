...
export const EXTRACTION_CONFIG: ExtractionConfig[] = [
  ...
  },
  {
    label: "Nama Gambar",
    pattern:
      /^(?:(?:COMPONENT|SCHEMATIC|WIRING|TB|SINGLE LINE|ARCHITECTURE)\s+(?:LAYOUT|INSTALLATION|DIAGRAM|TABLE|CONNECTION)\s+OF(?:\s+[A-Z0-9\s\-&',.\/]+)?|[A-Z0-9\s\-&',.\/\s]+)$/,
  ...
  },
  {
    label: "Type",
    pattern:
      /^(?:TC[1-3]?|M[1-2]?|T[1-3]?|T1'|KRL(?:\s+KCI)?|EMU(?:-R)?|612)(?:\s*[,;&]\s*(?:TC[1-3]?|M[1-2]?|T[1-3]?|T1'|KRL(?:\s+KCI)?|EMU(?:-R)?|612))*$/,
    zones: [
      {
        xStart: 1191 / 2,
        xEnd: 1191,
